...
std::shared_ptr<int32ClientMap> AsynPortClientContainer::getInt32Map(int address)
{
    return processMap<int32ClientMap>(int32Maps, address);
}

std::shared_ptr<float64ClientMap> AsynPortClientContainer::getFloat64Map(int address)
{
    return processMap<float64ClientMap>(float64Maps, address);
}

std::shared_ptr<octetClientMap> AsynPortClientContainer::getOctetMap(int address)
{
...
{
    std::shared_ptr<int32ClientMap> mapPtr = getInt32Map(address);
    // Check for the client
    if (mapPtr->count(paramName) == 0){
        // We need to create the client as it isn't stored
        (*mapPtr)[paramName] = std::shared_ptr<asynInt32Client>(new asynInt32Client(portName.c_str(), address, paramName.c_str()));
    }
...
{
    std::shared_ptr<float64ClientMap> mapPtr = getFloat64Map(address);
    // Check for the client
    if (mapPtr->count(paramName) == 0){
        // We need to create the client as it isn't stored
        (*mapPtr)[paramName] = std::shared_ptr<asynFloat64Client>(new asynFloat64Client(portName.c_str(), address, paramName.c_str()));
    }
    if ((*mapPtr)[paramName]->write(value) != asynSuccess){
        throw AsynException();
    }
...
{
    std::shared_ptr<octetClientMap> mapPtr = getOctetMap(address);
    size_t length = 0;
    size_t numWritten = 0;
    // Check for the client
    if (mapPtr->count(paramName) == 0){
        // We need to create the client as it isn't stored
        (*mapPtr)[paramName] = std::shared_ptr<asynOctetClient>(new asynOctetClient(portName.c_str(), address, paramName.c_str()));
    }
    length = value.size();
