...
asynNDArrayDriver drivers into a form that can be accessed by EPICS.
...
Int32, Float32, Float64]Array. These interfaces are supported by the
...
EPICS clients to receive this data they must be built with EPICS R3.14
(not R3.13), and the environment variable ``EPICS_CA_MAX_ARRAY_BYTES`` on
